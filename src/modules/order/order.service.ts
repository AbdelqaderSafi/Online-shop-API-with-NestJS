...
export class OrderService {
  ...
  async create(createOrderDto: CreateOrderDTO, userId: number | bigint) {
    ...
    if (products.length !== createOrderDto.length) {
    ...
    const orderTotalPrice = MoneyUtil.calculateTotalAmount(
      orderProductsData.map((orderProducts) => ({
        price: orderProducts.pricePerItem as Decimal,
        quantity: orderProducts.totalQty,
      })),
    );
  ...
  // helper method to map create order dto to order products create many input
  ...
  // ... (Lines 41-42: Function signature for createReturn)
  async createReturn(createReturnDto: CreateOrderReturnDTO, userId: bigint) {
    ...
    await this.prismaService.$transaction(async (prismaTX) => {
      // is order belong to user
      await prismaTX.order.findFirstOrThrow({
        where: {
          userId,
      ...
      // validate returns product ids already included in order && return qty is witin capacity to
